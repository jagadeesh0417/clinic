...
const treatment = getTreatmentBySlug("liquid-face-lift");
  ...
  return (
    <div className="min-h-screen bg-[#0F0F0E]">
        ...
        <div className="absolute bottom-0 left-0 right-0">
          <div className="mx-auto max-w-7xl px-4 pb-12 sm:px-6 lg:px-8">
            ...
            <p className="mt-3 max-w-2xl font-displayans text-base leading-relaxed text-[#F5F1E8]/60">
              A tailored combination of injectables to lift, contour, and refresh the face &mdash; no surgery, no downtime.
            </p>
            <div className="mt-5 flex flex-wrap gap-5">
              ...
              <div>
                ...
                <p className="font-displayans text-sm text-[#F5F1E8]/70">Immediate, refines over 1&ndash;2 weeks</p>
              </div>
              <div>
                <p className="font-['IBM_Plex_Mono'] text-[10px] tracking-wider uppercase text-[#4A5D3A]">Longevity</p>
                <p className="font-displayans text-sm text-[#F5F1E8]/70">12&ndash;18 months</p>
              </div>
              <div>
                <p className="font-['IBM_Plex_Mono'] text-[10px] tracking-wider uppercase text-[#4A5D3A]">Sessions</p>
                <p className="font-displayans text-sm text-[#F5F1E8]/70">1 session, refreshers as needed</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        ...
        {/* 1. What is a Liquid Face Lift? */}
        <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
          <Section title="What is a Liquid Face Lift?">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {[
                { label: "Non-Surgical", text: "Restores lost volume, softens lines, and lifts sagging areas without incisions" },
                { label: "Customized", text: "Customized to each patient's facial anatomy for balanced, natural-looking results" },
                { label: "Quick Procedure", text: "Performed in-clinic in under an hour with no general anesthesia" },
                { label: "Immediate Results", text: "Offers immediate improvement with continued refinement over the following days" },
              ].map(s => (
        ...
        <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true }} className="mt-14">
          <Section title="How It Works">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {[
                { step: "01", text: "Face is mapped to identify areas of volume loss, sagging, and dynamic wrinkles" },
                { step: "02", text: "Fillers are strategically placed to restore structure and lift" },
                { step: "03", text: "Neurotoxins soften expression lines in the upper face" },
                { step: "04", text: "Results visible immediately, refine over 1\u20132 weeks as swelling settles" },
              ].map(s => (
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              {treatment.gallery.map((src, i) => (
                <div key={i} className="group relative aspect-[4/3] overflow-hidden rounded-xl border border-[#F5F1E8]/[0.06]">
                  <img
                    src={src}
                    alt={`Liquid Face Lift gallery ${i + 1}`}
                    loading="lazy"
        ...
        >
          <p className="font-displayans text-xs text-[#F5F1E8]/30 leading-relaxed max-w-lg mb-6">
            Results and experience may vary. This treatment should only be performed by a qualified medical professional after consultation.
          </p>
