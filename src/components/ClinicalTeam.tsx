...
function DoctorAvatar({ name, size = "md" }: { name: string; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <div
      className={`${sizeMap[size]} rounded-full bg-gradient-to-br from-[#CBA135] to-[#E8C860] flex items-center justify-center text-[#050505] font-bold shrink-0 mx-auto mb-4`}
    >
      {initials}
    </div>
  );
...
export default function ClinicalTeam() {
  return (
    <section id="clinical-team" className="relative bg-[#050505] py-24 md:py-32 overflow-hidden">
      <div className="absolute top-1/3 left-1/4 w-96 h-96 bg-[#CBA135]/5 rounded-full blur-[150px] pointer-events-none" />
          ...
          {clinicalDoctors.map((doctor) => (
            ...
            >
              ...
              {doctor.qualifications && (
                <p
                  className="text-[#CBA135] text-xs font-medium mb-2"
                  style={{ fontFamily: "'Inter', sans-serif" }}
                >
                  {doctor.qualifications}
                </p>
